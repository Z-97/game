...
public class HttpHandler extends SimpleChannelInboundHandler<FullHttpRequest> {
	...
	// httpService权限ip集合
	private final Set<String> authIps;

	@Inject
	public HttpHandler(@Named("httpService.auth") boolean auth, @Named("httpService.ips") String ips) {
		this.auth = auth;
		this.authIps = new HashSet<>(Arrays.asList(ips.split(",")));
	}
	...
	protected void channelRead0(ChannelHandlerContext ctx, FullHttpRequest req) throws Exception {
		...
		if (auth && !authIps.contains(ip)) {
			HTTP_SERVICE_LOG.warn("ip[{}]没有权限发送Http请求", ip);
			ctx.close();
			return;
		}
		...
		HTTP_SERVICE_LOG.info("收到IP[{}]httpservice请求[{}]返回[{}]耗时[{}]毫秒", 
				ip, contentText, serviceResJson, System.currentTimeMillis() - now);
