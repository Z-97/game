...
public class MsgHandlerFactory {
	...
	public static void registerAllHandlers() throws Exception {
		try {
			...
			LOG.info("扫描handler实现类[{}]错误", e);
			throw new Exception("扫描handler实现类[{}]错误");
		}
	...
	public static final int getModuleId(String moduleKey) {
		...
		try {
			...
			LOG.error("加载[" + moduleKey + "]错误", e);
			System.exit(1);
		}
		LOG.info("加载模块[" + moduleId + "]" + moduleId);
		return moduleId;
	}
	...
	public static void loadConfig() {
		...
		try {
			...
			LOG.info("加载协议配置文件[" + PROTOCOL_CFG_FILE + "][{}]完成", jsonStr);
		} catch (IOException e) {
